...
// Relying on global gsap loaded via CDN
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const floatInput = document.getElementById('bronn-floating-auto-input') as HTMLInputElement;
    ...
    if (!visualSection || !floatInputBox || !floatInput || !macbookWindow || !initialState || !analyzingLoader || !overlay || !inputContainer) return;
    ...
    const runBronnSequence = async () => {
        if (bronnHasTriggered) return;
        ...
        while (true) {
            ...
            await typeText("Compare performance with top 3 competitors", floatInput);
    ...
    const initGsapReveal = () => {
        ...
        if (!textWrapper || !container) return;
